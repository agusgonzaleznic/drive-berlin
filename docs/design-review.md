...
- `exam.js render()`: `examReadiness(pool, state.quiz, state.exams, {lessonsDone, lessonsTotal})` becomes the
  ...
  "the real limit is 10 error points, and we want you comfortably under it before you pay". Do not touch
...
`allQuestions()`, show a card above the hero, "Attempt in progress, question 7 of 30, started 14 min ago", with **Resume** and a ghost **Discard**; otherwise fall through to a normal start. Add
...
scroll-snap-type: x proximity`), giving 44px instead of ~214px above the question and 44px tall cells, plus
...
1. `c-apply` step 2 tells him to bring "ADAC translation + classification", while `c-apply` tip 3 and
   ...
   constraint a line item, *printed on paper; the office's €6 on-site photo is digital and does not count*, instead of leaving it in a pitfall three blocks below. Re-verify that quote's source: `prerequisites.md:163-165`
...
on all 14 journey cards and task headers. **Ship the step-index migration in the same commit.** `toggleTaskStep` stores by array position, so any split lands his existing ticks on the wrong lines: add stable
...
| `data.test.mjs:60`, multi-correct > 25%, all-three < 10% | n/a | Don't rewrite question option sets |
