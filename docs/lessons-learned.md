...
included on purpose — they were more instructive than the successes.
...
- `min-width: 0` on `.module-top > div` — the **icon** is also a `div`, so `flex: 1`
...
- Opacity-based dimming (`.btn:disabled { opacity: .45 }`) composites **both** label
  and fill toward the page — 2.78:1. Disabled states need an explicit skin, and it
...
gold (one accent, used only where it means "do this next") and to confetti — seven
...
`currentColor` means an icon is automatically the colour of the text beside it — no
...
the test document the decision — `readiness refuses "ready" without passed mock exams`
